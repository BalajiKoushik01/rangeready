...
### 9.3 Instrument Auto-Discovery

On startup or on user request, the system performs a network scan:
1. Iterate over the configured IP subnet range
2. Attempt a VISA connection to each host on the standard VISA port (5025 for Keysight LAN instruments)
3. Send `*IDN?` to identify each responding device
4. Parse the response (format: `Manufacturer,Model,Serial,Firmware`) against the driver library
5. Register matched instruments in the asset registry with full identity metadata
...
- `backend/core/band_profiles.py` — band profile management
- `backend/core/limits.py` — pass/fail limit evaluation engine
...
- Parallel multi-DUT testing via switch matrix (future phase)
- Anechoic chamber OTA measurement integration
- AI-based anomaly detection or predictive analytics
- Mobile interface
